...
conf = yaml.load(o)
templateFilePath = './config/report_CD_gs_v4.docx'
saveFilePath = dir['director'] + '/' + conf['personalinfo']['GSID'] + '-' + conf['personalinfo']['name'] + '-' + 'CancerDrug_report-' + str(conf['otherinfo']['report_date']) + '.docx'
...
# 修改铂化物和紫杉类药物
for yw in med_stan['med'].index:
    if med_stan['med'][yw] == u'铂化物':
        med_stan['med'][yw] = '铂化物（以铂类为基础的化疗方案）'

    if med_stan['med'][yw] == u'紫杉类化合物':
        med_stan['med'][yw] = '紫杉类化合物（以紫杉类为基础的化疗方案）'

print 'change drug name ----- done'
# 输出!
...
# 四个化疗药物基因检测结果
if 'rs3918290' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs3918290')
    resultMap['#[DPYD]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[DPYD]#'] = '野生型'

if 'rs4148323' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs4148323')
    resultMap['#[UGT1A16]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[UGT1A16]#'] = '野生型'

if 'rs8175347' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs8175347')
    resultMap['#[UGT1A128]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[UGT1A128]#'] = '野生型'

if 'rs1800462' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs1800462')
    resultMap['#[TPMT1]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[TPMT1]#'] = '野生型'

if 'rs1800460' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs1800460')
    resultMap['#[TPMT2]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[TPMT2]#'] = '野生型'

if 'rs1142345' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs1142345')
    resultMap['#[TPMT3]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[TPMT3]#'] = '野生型'

if 'rs1065852' in var_selected['rsid'].values.tolist():
    tp = var_selected['rsid'].values.tolist().index('rs1065852')
    resultMap['#[CYP2D6]#'] = var_selected.iloc[tp,2]
else:
    resultMap['#[CYP2D6]#'] = '野生型'
    

print 'four important genes -----  done'
...
print 'task done     -------  Please delete extra lines!'
print 'df.txt        -------  gene describe file'
print 'med_stan.txt  -------  risk and sense'
print 'out_des.txt   -------  show by lines'
print 'out_all.xlsx  -------  result summary table'